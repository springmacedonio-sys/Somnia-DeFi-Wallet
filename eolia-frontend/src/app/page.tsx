...
export default function Home() {
  // 使用状态来存储客户端生成的随机值 / Use state to store client-side generated random values
  const [randomValues, setRandomValues] = useState<{
    starPositions: Array<{ left: string; top: string; delay: string; duration: string }>;
    particlePositions: Array<{ left: string; top: string; delay: string; duration: string; xOffset: string }>;
  }>({
    starPositions: [],
    particlePositions: []
  });

  // 在客户端生成随机值 / Generate random values on client side
  useEffect(() => {
    ...
    const generateParticlePositions = () => {
      return [...Array(25)].map(() => ({
        ...
        xOffset: `${Math.random() * 20 - 10}px`
      }));
    };

    setRandomValues({
      starPositions: generateStarPositions(),
      particlePositions: generateParticlePositions()
    });
  }, []);
    ...
    <main className="relative flex flex-col items-center h-screen w-screen bg-black overflow-hidden font-mono">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 opacity-70">
          {randomValues.starPositions.length > 0 ? randomValues.starPositions.map((star, i) => (
            <div
              key={i}
            ...
            // 服务端渲染时的占位符 / Placeholder during SSR
            [...Array(50)].map((_, i) => (
              <div
                key={i}
                className="absolute w-1 h-1 bg-cyan-400"
                style={{
                  left: `${(i * 2) % 100}%`,
                  top: `${(i * 3) % 100}%`,
                  animation: `twinkle ${2 + (i % 3)}s infinite ${(i % 2)}s`
                }}
              />
            ))
          )}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {randomValues.particlePositions.length > 0 ? randomValues.particlePositions.map((particle, i) => (
          <motion.div
            key={i}
